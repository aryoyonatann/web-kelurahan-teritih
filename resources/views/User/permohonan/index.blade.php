...
<head>
    ...
    <title>Permohonan Saya – Kelurahan Teritih</title>
...
.page-top-left h1 { font-size: 24px; font-weight: 800; color: #111827; margin-bottom: 2px; }
...
.btn-new:hover { background: #1a56db; transform: translateY(-1px); color: white; }
...
.stat-card { background: #fff; border: 1px solid #e5e7eb; border-radius: 12px; padding: 16px 18px; display: flex; align-items: center; gap: 12px; }
...
.table-card { background: #fff; border: 1px solid #e5e7eb; border-radius: 14px; overflow: hidden; box-shadow: 0 1px 6px rgba(0,0,0,.05); }
table { width: 100%; border-collapse: collapse; }
thead th { background: #f9fafb; padding: 11px 16px; text-align: left; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: .07em; color: #6b7280; border-bottom: 1px solid #e5e7eb; white-space: nowrap; }
...
.pill-ditolak  { background: #fee2e2; color: #7f1d1d; } .pill-ditolak::before  { background: #dc2626; }
...
.btn-detail { background: #eff6ff; color: #1c64f2; } .btn-detail:hover { background: #dbeafe; }
.btn-edit   { background: #f0fdf4; color: #15803d; } .btn-edit:hover { background: #dcfce7; }
.btn-delete { background: #fef2f2; color: #dc2626; } .btn-delete:hover { background: #fee2e2; }

.empty-state { text-align: center; padding: 64px 20px; }
.empty-icon { width: 64px; height: 64px; background: #f3f4f6; border-radius: 50%; display: inline-grid; place-items: center; margin-bottom: 16px; }
...
<div class="page-wrap">
    ...
    <div class="page-top">
        ...
        <a href="{{ route('user.permohonan.create') }}" class="btn-new">
    ...
    <div class="stats-row">
        <div class="stat-card">
            ...
            <div class="stat-info"><small>Total</small><strong>{{ $data->count() }}</strong></div>
        </div>
        <div class="stat-card">
            <div class="stat-icon yellow">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
            </div>
            <div class="stat-info"><small>Pending</small><strong>{{ $data->filter(fn($d) => !$d->approval || $d->approval->status === 'pending')->count() }}</strong></div>
        </div>
        <div class="stat-card">
    ...
    <div class="table-card">
        @if($data->isEmpty())
            <div class="empty-state">
                ...
                <a href="{{ route('user.permohonan.create') }}" class="btn-new" style="display:inline-flex">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="max-width:220px">{{ Str::limit($d->keperluan, 50) }}</td>
